...
$hasAssignedStudent = ($assignedStudents.data.Count -ge 1 -and $assignedStudents.data[0].registerNumber -eq "951021104001")
Report-Result "Staff Assigned Students DataGrid (GET /api/v1/staff/students)" $hasAssignedStudent "Returned $($assignedStudents.data.Count) student(s). RegNo: $($assignedStudents.data[0].registerNumber)"
...
$searchMatched = ($searchResp.data.Count -eq 1 -and $searchResp.data[0].registerNumber -eq "951021104001")
Report-Result "Staff Search by Register Number" $searchMatched "Search returned: $($searchResp.data.Count) item(s)"
...
$filterEmpty = ($filterResp.data.Count -eq 0)
Report-Result "Staff Scoped Filtering (Cannot expand beyond Section A)" $filterEmpty "Out-of-scope Section B returned $($filterResp.data.Count) students"
