...
// 1. Add Controllers with String Enum support
...
{
    throw new InvalidOperationException(
        $"JWT_SECRET in Production must be at least 32 characters long. Current length: {jwtSecret.Length}.");
}
else if (isProduction && jwtSecret == "Default_Dev_Secret_Key_At_Least_32_Characters_Long_For_Local_Dev!")
{
    throw new InvalidOperationException(
...
{
    options.RequireHttpsMetadata = isProduction; // HTTPS enforced in production
...
// Support cloud container dynamic port assignment (e.g., Vercel, Render sets $PORT)
