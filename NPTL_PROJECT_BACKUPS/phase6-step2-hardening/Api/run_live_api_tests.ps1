...
try {
    ...
    $studentsList = if ($res.data.items) { $res.data.items } else { $res.data }
    $hasStudent1 = ($studentsList | Where-Object { $_.registerNumber -eq "951021104001" }) -ne $null
    $hasStudent2 = ($studentsList | Where-Object { $_.registerNumber -eq "951021104002" }) -ne $null
    $pass = ($res.success -eq $true) -and $hasStudent1 -and (-not $hasStudent2)
    Record-Test 11 "Staff -> /staff/students (Scoped)" "Only assigned students" "Count=$($studentsList.Count)" $pass "Contains Year 3 ($hasStudent1), Excludes Year 1 ($((-not $hasStudent2)))"
} catch {
    Record-Test 11 "Staff -> /staff/students (Scoped)" "200 OK" $_.Exception.Message $false ""
}

# ----------------------------------------------------------------------------
# Test 12: Staff -> unassigned student /staff/students/{id} (Forbidden / NotFound)
# ----------------------------------------------------------------------------
try {
    $headers = @{ Authorization = "Bearer $staffToken" }
    ...
    Record-Test 12 "Staff -> unassigned student" "403 Forbidden / 404 Not Found" "200 OK" $false "Staff should not access unassigned student"
} catch {
    $code = $_.Exception.Response.StatusCode.value__
    $pass = ($code -eq 403 -or $code -eq 404)
    Record-Test 12 "Staff -> unassigned student" "403 Forbidden / 404 Not Found" "$code" $pass "Cross-scope student access correctly blocked"
}
