...
public class ApiClient : IApiClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(classSection))
            queryParams.Add($"classSection={Uri.EscapeDataString(classSection.Trim())}");
        ...
        var result = await SendAsync<List<StudentProfileResponse>>(
            HttpMethod.Get, 
            url, 
            null, 
            includeAuth: true, 
        ...
        return result ?? new List<StudentProfileResponse>();
    }
